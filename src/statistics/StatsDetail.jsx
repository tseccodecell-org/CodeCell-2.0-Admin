...
function generateStats(week) {
  const rng = seededRng(week.id * 997 + week.problems.length * 31)
  ...
  const participants = MOCK_NAMES.slice(0, count).map((name, i) => ({
    id: i + 1, name,
  ...
  const problemStats = problems.map((prob, idx) => {
...
const VERDICT_DOT = { AC: 'bg-emerald-500', WA: 'bg-rose-500', TLE: 'bg-amber-500', CE: 'bg-slate-400', RE: 'bg-purple-500' }
...
const DIFF_BAR = { Easy: '#10b981', Medium: '#f59e0b', Hard: '#f43f5e' }
...
function SearchBar({ value, onChange, placeholder = 'Search participant...' }) {
  return (
...
function ParticipantPanel({ participant, problems, submissions, onClose, onSubClick, weeks, currentWeekId }) {
  ...
  const counts = { all: mySubs.length }
        ...
        <div className="px-6 py-3 border-b border-slate-100 bg-slate-50/60 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              onChange={e => { setSelectedWeekId(Number(e.target.value)); setVerdictFilter('all') }}
...
export default function StatsDetail() {
  ...
  if (!week) return <div className="flex items-center justify-center h-full text-slate-400 text-sm">Contest not found.</div>
  ...
  function handleSubFilter(v) { setSubFilter(v); setSubPage(1) }
  function openSubmission(sub) { setSelectedParticipant(null); setSelectedSubmission(sub) }

  return (
    <div className="flex flex-col min-h-full">
      {/* Breadcrumb */}
      <div className="h-16 bg-white border-b border-slate-200/80 flex items-center px-8 gap-2 shrink-0">
        <Link to="/statistics" className="text-sm text-slate-400 hover:text-slate-700 transition-colors font-medium">Analytics</Link>
                    ...
                    <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
                      ...
                      <ResponsiveContainer width="100%" height={220}>
                        <AreaChart data={dropoutData} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
                          ...
                          <Legend wrapperStyle={{ paddingTop: 16, fontSize: 12 }} formatter={v => <span style={{ color: '#64748b' }}>{v}</span>} />
                ...
                <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
                  ...
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart data={langData} margin={{ top: 4, right: 16, left: 0, bottom: 0 }} barGap={4}>
                      ...
                      <Legend wrapperStyle={{ paddingTop: 12, fontSize: 12 }} formatter={v => <span style={{ color: '#64748b' }}>{v}</span>} />
